    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # Print time (inference + NMS) 
                #print(f'{label1} : {s} ')
                #print(f'central:{central} width:{width} hello:{hellocount} gooodbye:{goodbyecount}') #デバッグ
                ...
                if (hellocount >= 7 and width[-1] > 450) or (goodbyecount >= 7 and central[-1] < 140 and width[-1] > 130) or (goodbyecount <= -7 and goodbyecount > -20 and width[-1] > 130):
                    ...
                    if putcount1 < 0 and putcount2 < 0 and putcount3 < 0:
                        if number == 1:
                            print(f"{dt_now.strftime('%m-%d %H:%M')}面白いから本の紹介をしてみない?")
                            talksyaberun("こんにちは、面白いから本の紹介をしてみない?")
                            putcount1 = 50
                        if number == 2:
                            print(f"{dt_now.strftime('%m-%d %H:%M')}新しく、コンテンツ、が追加されたよ")
                            talksyaberun("こんにちは、新しく、ハイキュー、が追加されたよ")
                            putcount2 = 50
                        if number == 3:
                            print(f"{dt_now.strftime('%m-%d %H:%M')}本を紹介するロボットしゃべるんです")
                            talksyaberun("こんにちは、本を紹介するロボットしゃべるんです")
                            putcount3 = 50
                        hellocount = -1000
                        goodbyecount = -1000
